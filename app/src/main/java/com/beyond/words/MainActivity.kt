...
class MainActivity : AppCompatActivity() {
    ...
    private fun decode() {
        try {
            ...
            status.text = "Decode failed: ${e.message}"
        }
    }
    ...
    override fun onActivityResult(req: Int, result: Int, data: Intent?) {
        ...
        try {
            ...
            status.text = "Operation failed: ${e.message}"
        }
    }
...
private object BeyondCodec {
    ...
    fun decode(text: String): Decoded {
        ...
        if (all.size < fixed + 4) throw IllegalArgumentException("Word sequence is too short")
        var p = 0
        fun u8() = all[p++].toInt() and 255
        ...
        if (String(all.copyOfRange(0, 3), Charsets.US_ASCII) != MAGIC) throw IllegalArgumentException("Invalid container")
        p = 3
        val ver = u8()
        ...
        val rawLen = u64()
        val payLen = u64()
        val sha = all.copyOfRange(p, p + 32)
        p += 32
        if (payLen < 0 || payLen > Int.MAX_VALUE) throw IllegalArgumentException("Image is too large")
        if (p + nl + ml + payLen.toInt() + 4 > all.size) throw IllegalArgumentException("Truncated word sequence")
    ...
    private fun inflate(b: ByteArray): ByteArray {
        ...
        try {
            ...
            if (e is IllegalArgumentException && e.message?.startsWith("Missing") == true) throw e
            if (e is IllegalArgumentException && e.message?.startsWith("Compressed") == true) throw e
